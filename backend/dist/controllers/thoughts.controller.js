...
async function getThoughts(req, res) {
    try {
        ...
        const thoughtsQuery = firebaseAdmin_1.adminFirestore.collection('thoughts');
        if (dateFilter === 'all') {
...
async function createThought(req, res) {
    try {
        ...
        const existingThoughtSnap = await firebaseAdmin_1.adminFirestore
            .collection('thoughts')
            .where('publishDate', '==', today)
            .get();
        if (!existingThoughtSnap.empty) {
            ...
            await firebaseAdmin_1.adminFirestore.collection('thoughts').doc(existingDocId).update({
                text: thought,
        ...
        const docRef = await firebaseAdmin_1.adminFirestore.collection('thoughts').add(thoughtDoc);
        res.status(201).json({
            success: true,
...
async function deleteThought(req, res) {
    try {
        const thoughtId = req.query.id;
        if (!thoughtId) {
            res.status(400).json({ success: false, message: 'Thought ID is required' });
            return;
        }
        await firebaseAdmin_1.adminFirestore.collection('thoughts').doc(thoughtId).delete();
