...
exports.createDirectSuperadmin = createDirectSuperadmin;
exports.promoteToSuperadmin = promoteToSuperadmin;
exports.transitionRoles = transitionRoles;
exports.getRoleDistribution = getRoleDistribution;
exports.assignConversation = assignConversation;
exports.backfillPublicIds = backfillPublicIds;
exports.getPublicIdStatus = getPublicIdStatus;
exports.cleanConversations = cleanConversations;
exports.getConversationStatus = getConversationStatus;
exports.setupCounters = setupCounters;
...
async function listUsers(_req, res) {
    try {
        const usersSnapshot = await firebaseAdmin_1.adminFirestore.collection('users').get();
        const users = [];
...
async function createTeacher(req, res) {
    try {
        ...
        const userRecord = await firebaseAdmin_1.adminAuth.createUser({ email, password, displayName: name || 'Teacher' });
        const counterDoc = await firebaseAdmin_1.adminFirestore.collection('counters').doc('teacherCounter').get();
        let counterValue = 1;
        if (counterDoc.exists) {
            const counterData = counterDoc.data();
            if (counterData && typeof counterData.value === 'number')
                counterValue = counterData.value + 1;
            await firebaseAdmin_1.adminFirestore.collection('counters').doc('teacherCounter').update({ value: counterValue });
        }
        else {
            await firebaseAdmin_1.adminFirestore.collection('counters').doc('teacherCounter').set({ value: counterValue });
        }
        const publicId = `TCH-${counterValue.toString().padStart(4, '0')}`;
        await firebaseAdmin_1.adminFirestore.collection('users').doc(userRecord.uid).set({
...
async function createSuperadmin(req, res) {
    try {
        ...
        let userQuery;
        if (uid) {
            userQuery = firebaseAdmin_1.adminFirestore.collection('users').doc(uid);
        }
        else {
            const usersSnapshot = await firebaseAdmin_1.adminFirestore.collection('users').where('email', '==', email).get();
        ...
        const updatedData = { ...userData, role: 'super_admin', updatedAt: firestore_1.FieldValue.serverTimestamp() };
        if (!userData.publicId) {
            updatedData.publicId = await (0, idGenerator_1.generatePublicId)('super_admin');
        }
...
async function getSuperadmins(_req, res) {
    try {
        const snapshot = await firebaseAdmin_1.adminFirestore.collection('users').where('role', '==', 'super_admin').get();
        const superAdmins = [];
...
async function createDirectSuperadmin(req, res) {
    try {
        const { email, password, name } = req.body;
        if (!email || !password) {
            res.status(400).json({ success: false, message: 'Email and password are required' });
        ...
        const userRecord = await firebaseAdmin_1.adminAuth.createUser({ email, password, displayName: name || 'Super Admin' });
        const counterDoc = await firebaseAdmin_1.adminFirestore.collection('counters').doc('superAdminCounter').get();
        let counterValue = 1;
        if (counterDoc.exists) {
            const counterData = counterDoc.data();
            if (counterData && typeof counterData.value === 'number')
                counterValue = counterData.value + 1;
            await firebaseAdmin_1.adminFirestore.collection('counters').doc('superAdminCounter').update({ value: counterValue });
        }
        else {
            await firebaseAdmin_1.adminFirestore.collection('counters').doc('superAdminCounter').set({ value: counterValue });
        }
...
async function promoteToSuperadmin(req, res) {
    try {
        ...
        const userData = userDoc.data();
        await userDoc.ref.update({ role: 'super_admin', updatedAt: firestore_1.FieldValue.serverTimestamp() });
...
async function transitionRoles(req, res) {
    try {
        ...
        for (const transition of transitions) {
            ...
            if (usersSnapshot.empty)
                continue;
            const userDoc = usersSnapshot.docs[0];
            const updateData = { role: newRole, updatedAt: firestore_1.FieldValue.serverTimestamp() };
            if (publicId)
...
async function getRoleDistribution(_req, res) {
    try {
        const usersSnapshot = await firebaseAdmin_1.adminFirestore.collection('users').get();
        const roleDistribution = { student: 0, admin: 0, teacher: 0, super_admin: 0, total: 0 };
...
async function assignConversation(req, res) {
    try {
        ...
        const conversationRef = firebaseAdmin_1.adminFirestore.collection('conversations').doc(conversationId);
        ...
        let updateData = { updatedAt: firestore_1.FieldValue.serverTimestamp() };
        if (teacherId) {
            let teacherName = null;
            let teacherPublicId = null;
            try {
                const teacherDoc = await firebaseAdmin_1.adminFirestore.collection('users').doc(teacherId).get();
            ...
            updateData = {
                ...
                assignedTeacherName: teacherName, assignedBy, assignedAt: firestore_1.FieldValue.serverTimestamp(),
...
async function backfillPublicIds(_req, res) {
    try {
        const usersSnapshot = await firebaseAdmin_1.adminFirestore.collection('users').get();
        if (usersSnapshot.empty) {
            res.json({ success: true, message: 'No users found', usersUpdated: 0 });
            return;
        }
        let usersUpdated = 0;
        const errors = [];
        ...
        for (let i = 0; i < users.length; i += batchSize) {
            ...
            for (const userDoc of batch) {
                try {
                    ...
                    const publicId = await (0, idGenerator_1.generatePublicId)(role);
                    await firebaseAdmin_1.adminFirestore.collection('users').doc(userDoc.id).update({ publicId, updatedAt: firestore_1.FieldValue.serverTimestamp() });
                    usersUpdated++;
                }
                catch (error) {
...
async function getPublicIdStatus(_req, res) {
    try {
        const usersSnapshot = await firebaseAdmin_1.adminFirestore.collection('users').get();
...
async function cleanConversations(_req, res) {
    try {
        const conversationsSnapshot = await firebaseAdmin_1.adminFirestore.collection('conversations').get();
        if (conversationsSnapshot.empty) {
            res.json({ success: true, message: 'No conversations to clean', conversationsDeleted: 0, messagesDeleted: 0 });
            return;
        }
        ...
        for (let i = 0; i < conversations.length; i += 10) {
            ...
            for (const conversationDoc of batch) {
                try {
                    const messagesSnapshot = await firebaseAdmin_1.adminFirestore.collection('conversations').doc(conversationDoc.id).collection('messages').get();
                    if (!messagesSnapshot.empty) {
                        const messageBatch = firebaseAdmin_1.adminFirestore.batch();
                        messagesSnapshot.forEach(msgDoc => messageBatch.delete(msgDoc.ref));
                        await messageBatch.commit();
                        messagesDeleted += messagesSnapshot.size;
                    }
                    await firebaseAdmin_1.adminFirestore.collection('conversations').doc(conversationDoc.id).delete();
...
async function getConversationStatus(_req, res) {
    try {
        const conversationsSnapshot = await firebaseAdmin_1.adminFirestore.collection('conversations').get();
        let totalMessages = 0;
        ...
        for (const doc of sample) {
            const messagesSnapshot = await firebaseAdmin_1.adminFirestore.collection('conversations').doc(doc.id).collection('messages').limit(100).get();
...
async function setupCounters(_req, res) {
    try {
        const counters = [
        ...
        for (const counter of counters) {
            const ref = firebaseAdmin_1.adminFirestore.collection('counters').doc(counter.id);
            batch.set(ref, { lastNumber: counter.lastNumber, prefix: counter.prefix, createdAt: firestore_1.FieldValue.serverTimestamp(), updatedAt: firestore_1.FieldValue.serverTimestamp() });
        }
        await batch.commit();
...
async function getCounters(_req, res) {
    try {
        const snapshot = await firebaseAdmin_1.adminFirestore.collection('counters').get();
        const counters = [];
...
async function setupUsers(_req, res) {
    res.json({
        success: true,
...
async function getSetupInfo(_req, res) {
    res.json({
...
async function setupCollections(_req, res) {
    try {
        await firebaseAdmin_1.adminFirestore.collection('questions').doc('sample-question').set({
        ...
        await firebaseAdmin_1.adminFirestore.collection('thoughts').doc('sample-thought').set({
...
async function getTestUsers(_req, res) {
    res.json({
