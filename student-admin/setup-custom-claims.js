// Script to set custom claims for users who don't have them
// Usage: node setup-custom-claims.js

const admin = require('firebase-admin');

// Initialize Firebase Admin SDK
const serviceAccount = {
  projectId: 'student-portal-fab55',
  clientEmail: 'firebase-adminsdk-fbsvc@student-portal-fab55.iam.gserviceaccount.com',
  privateKey: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
};

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  projectId: 'student-portal-fab55'
});

async function setupCustomClaims() {
  console.log('🔧 Setting up custom claims for users...\n');
  
  try {
    // Define user roles and permissions
    const userRoles = {
      'admin@admin.com': {
        role: 'admin',
        permissions: ['all']
      },
      '2@admin.com': {
        role: 'admin', 
        permissions: ['all']
      },
      'rahul@student.com': {
        role: 'student',
        permissions: ['chat_with_admin', 'view_questions']
      },
      'likhith@student.com': {
        role: 'student',
        permissions: ['chat_with_admin', 'view_questions']
      }
    };

    let updatedCount = 0;
    let skippedCount = 0;

    // Process each user
    for (const [email, claims] of Object.entries(userRoles)) {
      try {
        console.log(`👤 Processing: ${email}`);
        
        // Get user record
        const userRecord = await admin.auth().getUserByEmail(email);
        
        // Check if user already has custom claims
        const existingClaims = userRecord.customClaims || {};
        const needsUpdate = 
          existingClaims.role !== claims.role || 
          JSON.stringify(existingClaims.permissions) !== JSON.stringify(claims.permissions);
        
        if (needsUpdate) {
          // Set custom claims
          await admin.auth().setCustomUserClaims(userRecord.uid, claims);
          console.log(`   ✅ Updated custom claims for ${email}`);
          console.log(`      Role: ${claims.role}`);
          console.log(`      Permissions: ${JSON.stringify(claims.permissions)}`);
          updatedCount++;
        } else {
          console.log(`   ⏭️  Skipping ${email} - already has correct claims`);
          skippedCount++;
        }
        
      } catch (error) {
        console.log(`   ❌ Error processing ${email}: ${error.message}`);
      }
    }
    
    console.log(`\n📊 Summary:`);
    console.log(`   Users Updated: ${updatedCount}`);
    console.log(`   Users Skipped: ${skippedCount}`);
    console.log(`   ✅ Custom claims setup complete!`);
    
  } catch (error) {
    console.error('❌ Error setting up custom claims:', error.message);
  }
  
  process.exit(0);
}

setupCustomClaims().catch(console.error);
